...
/*
 * If you want to use the managed dll, you must use this namespace.
 */
using MantisLOD;
using UnityEditor;
using UnityEngine;
...
namespace MantisLODEditor
{
	...
	public class InnerMantisLODEditorProfessional: Editor {
		...
		/*
         * If you want to simplify meshes at runtime, please use the managed dll.
         * Managed dll can run on all platforms, but it is slower than native dll.
         * To use managed dll, You just need to add prefix of 'MantisLODSimpler.'
         * before the native APIs.
         */
		private bool use_managed_dll = false;
		...
		override public void OnInspectorGUI() {
			...
			if(target) {
				if(Mantis_Meshes != null) {
					show_title = EditorGUILayout.Foldout(show_title, "Mantis LOD Editor - Professional Edition V6.2");
					if(show_title) {
						...
						if (GUI.changed) {
							...
							if(save_quality != quality) {
								...
								foreach (Mantis_Mesh child in Mantis_Meshes) {
									...
									if(child.index != -1 && (!use_managed_dll ? get_triangle_list(child.index, quality, child.out_triangles, ref child.out_count) : MantisLODSimpler.get_triangle_list(child.index, quality, child.out_triangles, ref child.out_count)) == 1) {
